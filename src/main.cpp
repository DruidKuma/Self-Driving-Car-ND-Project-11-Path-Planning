        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2')
        {
            ...
            if (s != "") 
            {
                ...
                if (event == "telemetry")
                {
                    ...
                    // distance limits
                    double max_distance_front = 20;
                    double max_distance_back = 10;

                    // to track if another car is too close
                    bool close_front = false; 
                    bool close_left = false; 
                    bool close_right = false;

                    // gaps on sides
                    double dist_left = 1000;
                    double dist_right = 1000;

                    // for following front car if no way to change line
                    double ref_speed = 0; 

                    // read sensor fusion data
                    for (int i = 0; i < sensor_fusion.size(); i++)
                    {
                      // vehicle data (x, y, s, d, speed, lane, distance to our car)
                      ...
                      double veh_speed = sqrt(pow(veh_vx, 2) + pow(veh_vy, 2));

                      int veh_lane;
                      if (veh_d > 4 and veh_d <= 8) veh_lane = 1;
                      else if (veh_d > 8 and veh_d <= 12) veh_lane = 2;
                      else veh_lane = 0;
                      
                      // add prediction for the next time tick
                      veh_s += double(prev_size) * 0.02 * veh_speed;

                      double distance = veh_s - car_s;

                      // if vehicle is close
                      if (distance > -max_distance_back and distance < max_distance_front)
                      {
                        if (veh_lane == lane - 1) close_left = true;
                        if (veh_lane == lane + 1) close_right = true;
                      }
                      // check sides
                      if (veh_lane == lane - 1 and distance < dist_left and distance > 0) dist_left = distance;
                      if (veh_lane == lane + 1 and distance < dist_right and distance > 0) dist_right = distance;
                      
                      if (veh_lane == lane and distance > 0 and distance < max_distance_front)
                      {
                        close_front = true;
                        ref_speed = veh_speed;
                      }
                    ...
                    // check in what lane we are now
                    if (lane == 0) close_left = true;
                    else if (lane == 2) close_right = true;
                    ...
                    if (close_front)
                    {
                      ...
                      {
                          if (dist_left > dist_right) lane--;
                          else lane++;
                      }
                      else if (!close_left and lane != 0) lane--;
                      else if (!close_right and lane != 2) lane++;

                      if (car_speed > ref_speed) desired_speed -= 0.224;
                      else desired_speed += 0.224;
                    }
                    else if (desired_speed < 49.7) desired_speed += 0.224;
